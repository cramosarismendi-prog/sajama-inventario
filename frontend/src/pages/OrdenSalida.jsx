...
export default function OrdenSalida() {
  ...
  const { escuchando: scannerActivo, activar: activarScanner, desactivar: desactivarScanner } = useScannerQR(async (textoQR) => {
    ...
    // Buscar item en catálogo por código
    const item = items.find(i => String(i.codigo) === String(parsed.codigo))

    // Traducir ANTES de setFilas para no usar await dentro del setter
    ...
    // Autocompletar la fila activa
    // parsed.serie = numero de serie del QR (ej: 11212087)
    // parsed.descripcion = descripcion en espanol (ej: RETEN 130 X 120 X 7)
    setFilas(prev => {
      const n = [...prev]
      n[filaActiva] = {
        ...n[filaActiva],
    ...
    toast.success('QR leido fila ' + (filaActiva+1) + ': ' + (item?.descripcion || parsed.descripcion))
    desactivarScanner()
    setFilaActiva(prev => Math.min(prev + 1, MAX_FILAS - 1))
  })
...
<div class="campos">
  <div class="row-campo">
    <div class="campo"><b>单据号码 Numero de solicitud:</b><span class="val">${numSolicitud}</span></div>
      ...
      <div className="card p-0 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            ...
            <tbody>
              {filas.map((f, idx) => (
                <tr key={idx} className={`border-b border-gray-100 ${filaActiva === idx && scannerActivo ? 'bg-primary-pale' : ''}`}>
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-3 gap-0 border border-gray-300 rounded-lg overflow-hidden text-xs">
          <div className="p-3 border-r border-gray-300 space-y-3">
            ...
            <div><p className="font-bold">区域负责人批准 / Aprobado Por (JEFE DE AREA):</p><div className="border-b border-gray-400 mt-4"/><p className="text-gray-400">日 Fecha: 年___ 月___</p></div>
            <div><p className="font-bold">仓库管理层批准 / Aprobado Por (GERENCIA):</p><div className="border-b border-gray-400 mt-4"/><p className="text-gray-400">日 Fecha: 年___ 月___</p></div>
          </div>
